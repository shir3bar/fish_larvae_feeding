...
class UncuratedLabeler(Labeler):
    ...
    LABEL_LIST = ['swim','strike','strike no/small jump', 'Spit/I&O','pre-strike','mouth opening',
                  'small fish','big fish partially cut',
                  'multiple fish', 'fish offcenter','just head', 'just tail','blurry',
                  'very blurry', 'no fish','turn','sharp turn', 'crazy bout','fast swim','side swim',
                  'interrupted swimming', 'overexposed image', 'reverse swim','dark image',
                  'dark artifact','light artifact', 'occlusion','floating', 'camera movement',
                  'lens scratches', 'lots of artemia',
                  'ventral','weird angle','late entry','unusable']
    LABEL_MULTICHOICE = True
    ...
    def handle_missing_log(self):
        ...
        for filename in ['preds_without_removed.csv','preds.csv']:
            if os.path.isfile(os.path.join(self.directory,filename)):
                ...
                self.log_filepath = os.path.join(self.directory, 'labeled_preds.csv')
                if self.multichoice:
                ...
                msg = f'Loaded regular {filename} instead of the removed version, saving it as a new labeled_preds.csv file'
                break
        if len(msg)==0:
